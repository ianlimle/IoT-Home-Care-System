...
def on_message(client, userdata, msg):
    ...
    warning_dict= {"rpi2/sensor/flame": ["None detected"],
                    "rpi2/sensor/pot": ["Detected", "None Detected"],
                    "rpi2/sensor/gas": ["High"],
                    "rpi2/sensor/flowrate": ["High", "Low"]}

    safe_dict= {"rpi2/sensor/pot": ["Detected"],
                "rpi2/sensor/gas": ["Low"],
                "rpi2/sensor/flowrate": ["High", "Low"]}
    ...
    for key, value in warning_dict:
        for element in value:
            if k == key and v == element:
                print("calling on script to turn off gas valve")
                ###### add command function to execute ######
                triggered("Open flame detected. Turning off gas valve")
                ##turn_off_gasvalve()

    for keys, values in safe_dict:
        for elements in values:
            if k == keys and v == elements:
                print("calling on script to turn on gas valve")
                triggered("Turning on gas valve")
                #turn_on_gas_valve()


    ########################### OR #####################################################################

    if msg.topic.decode() == "rpi2/sensor/flame" and msg.payload.decode() == "None detected" :
        print("calling on script to turn off gas valve...")
        ##### add command function to execute ######
        triggered("Open flame detected")
        #turn_off_gasvalve()
        
    #elif: #set conditions to trigger on/off state for gas valve  
          #
          #
          #
          #
          #
    #else:
        #set conditions to trigger on/off state for gas valve
        #
        #
        #
        #
...
#gasvalve payload is a boolean: ON/OFF
gas_valve_status= "ON"

""" SENSORS PUBLISHES PAYLOADS TO TOPICS """
...
################################# SWITCH SUBSCRIBES TO CORRESPONDING TOPICS AS ABOVE ############################

#subscribe and listen to the specific MQTT topic
#allows multiple topic subscriptions in a single subscription command
#on the configuration.yaml file, the corresponding topic for the switch is under command_topic
client.subscribe("rpi1/switch/gasvalve", 0)

################################# SWITCH PUBLISHES PAYLOAD TO ITS OWN UNQIUE TOPIC ON HOMEASSISTANT #############

#on the configuration.yaml file, the corresponding topic for the switch is under state_topic
client.publish("rpi2/switch/gasvalve", str(gas_valve_status), qos=0, retain=False)
